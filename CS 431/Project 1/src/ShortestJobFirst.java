...
class ShortestJobFirst extends SchedulerAlg {

  private PriorityQueue<Process> processes = new PriorityQueue<>();

  ShortestJobFirst(ArrayList<Process> list, String fileName) throws IOException {
    processes.addAll(list);
    this.fileName = fileName;
    writeHeaderToFile(fileName);
    totalProcesses = list.size();
  }
  ...
  void writeFinishedProcessToFile(String fileName, int pid) throws IOException {
    System.out.println("Process " + pid + " finished, writing to SJF-" + fileName + ".csv");
    BufferedWriter bw = new BufferedWriter(new FileWriter("./output/SJF-" + fileName + ".csv", true));
  ...
  void writeAverageToFile(String fileName, int avgCompletionTime) throws IOException {
    System.out.println("Processes finished. Writing average completion time to SJF-" + fileName + ".csv");
    BufferedWriter bw = new BufferedWriter(new FileWriter("./output/SJF-" + fileName + ".csv", true));
  ...
  void writeSwitchToFile(String fileName, int prevPid, int newPid, int beginningCpuTime, int endCpuTime) throws IOException {
    System.out.println("Switching processes, writing to SJF-" + fileName + ".csv");
    BufferedWriter bw = new BufferedWriter(new FileWriter("./output/SJF-" + fileName + ".csv", true));
  ...
  void writeDataToFile(String fileName, int pid, int startingCPU, int cpuTime, int burstTime, int endingBurstTime, int completionTime) throws IOException {
    System.out.println("Writing data for process " + pid + " to SJF-" + fileName + ".csv");
    BufferedWriter bw = new BufferedWriter(new FileWriter("./output/SJF-" + fileName + ".csv", true));
  ...
  void writeHeaderToFile(String fileName) throws IOException {
    ...
    BufferedWriter bw = new BufferedWriter(new FileWriter("./output/SJF-" + fileName + ".csv", false));
